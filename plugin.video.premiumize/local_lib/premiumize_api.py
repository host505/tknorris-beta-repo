...
class Premiumize_API():
    ...
    def add_torrent(self, torrent, folder_id=None):
        url = '/api/transfer/create'
        params = {'type': 'torrent'}
        if folder_id is not None:
            params['folder_id'] = folder_id
        
        if torrent.startswith('http') or torrent.startswith('magnet'):
            data = {'src': torrent}
            ...
            multipart_data += '\n--%s--\n' % (BOUNDARY)
            log_utils.log('Multipart Data: |%s|%s|' % (BOUNDARY, len(torrent)), log_utils.LOGDEBUG)
    ...
    def __call_premiumize(self, url, params=None, data=None, multipart_data=None):
        ...
        log_utils.log('Premiumize Call: Url: |%s| Data: |%s|' % (url, data), log_utils.LOGDEBUG)
